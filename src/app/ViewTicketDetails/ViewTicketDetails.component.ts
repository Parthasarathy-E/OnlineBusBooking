...
element: any;
  ...
  cancelTicket(){
    ...
    for(let index = 0 ; index < this.ticketInfo.busDetails.Seats.length ; index++){
      if(seatNo.includes(this.ticketInfo.busDetails.Seats[index].index)){
        this.ticketInfo.busDetails.Seats.splice(index, 1);
        index--;
      }
    }
    this.http.patch<any>("http://localhost:3000/BusList/" + this.ticketInfo.busDetails.id , this.ticketInfo.busDetails).subscribe(res => console.log(res));
